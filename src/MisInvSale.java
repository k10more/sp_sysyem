...
import javax.swing.border.*;
 ...
 class MisInvSale extends JFrame  implements ActionListener 
{
	...
	*/
	...
	{	
		...
		*/
		...
		try
		{
			...
			rs=st.executeQuery("select  DISTINCT SInvoice_Date from SInvoice order by SInvoice_Date ");
			while(rs.next())
			{
	...
	public void actionPerformed(ActionEvent e)
	{
			...
			else
			{
				...
				else
				{
					...
					//if(supNm.equals(no)  &&  OrdSta.equals(no))
					{
						try
						{
							rs2=st2.executeQuery("select * from SInvoice where  SInvoice_Date  between  '"+dtFrom+"'   and  '"+dtTo+"'  ");
							while(rs2.next())
							{
					...
					//if(!(supNm.equals(no)  &&  OrdSta.equals(no)))
					{
						...
						try
						{
							...
							rs2=st2.executeQuery("select SOrder_No from SOrder  where SQuo_No in (select Quo_No from Quotation where QuoCust_Id = '"+suptemp+"') ");
							rs2.next();
							suptemp=rs2.getString(1);

							rs2=st2.executeQuery("select * from SInvoice where SIOrder_No='"+suptemp+"'  and  SIStatus="+statemp+"  and  SInvoice_Date  between  '"+dtFrom+"'   and  '"+dtTo+"'  ");
							while(rs2.next())
							{
								ONo.addItem(rs2.getString(1));
								recCnt++;
					...
					//if( !(supNm.equals(no))  &&  OrdSta.equals(no) && b==true )
					{
						...
						//{
							...
							rs5=st5.executeQuery("select SOrder_No from SOrder  where SQuo_No in (select Quo_No from Quotation where QuoCust_Id = '"+suptemp+"') ");
							while(rs5.next())
							{
							suptemp=rs5.getString(1);
							System.out.println(suptemp);

							rs2=st2.executeQuery("select * from SInvoice where SIOrder_No='"+suptemp+"'  and  SInvoice_Date  between  '"+dtFrom+"'   and  '"+dtTo+"'  ");
							while(rs2.next())
							{
								ONo.addItem(rs2.getString(1));
								recCnt++;
					...
					//if(!(OrdSta.equals(no))  && supNm.equals(no)  && b==true)
						{
							try
							{
								...
								rs2=st2.executeQuery("select * from SInvoice where SIStatus="+statemp+"  and    SInvoice_Date  between  '"+dtFrom+"'   and  '"+dtTo+"'  ");
		...
		if(e.getSource()==rpt)
		{
			...
			ReportSInv b = new ReportSInv(ori,pnm,ppri,qord,qinv);
		}
		...
		if(e.getSource()==ONo)
		{
			...
			try
			{
				...
				rs2=st2.executeQuery("select * from SInvoice where SInvoice_No='"+num+"' " );
				rsmd2=rs2.getMetaData();
				rs2.next();
					//rs2.absolute(no);
					//rs2.beforeFirst();
					...
					rs4=st4.executeQuery("select  QuoCust_Id from Quotation  where  Quo_No in ( select SQuo_No from SOrder where SOrder_No= '"+rs2.getString(3)+"')");
					rs4.next();
					String custNm=rs4.getString(1);
					rs4=st4.executeQuery("select  Cust_Name from Customer  where  Cust_Id ='"+custNm+"' ");
					rs4.next();
				...
				try
				{
					if(rs2.getInt(7)==2)
					{
						rs3=st3.executeQuery("select * from SMRet where SMRInvoice_No='"+ord+"' ");
						rs3.next();
						pm = pm + rs3.getDouble(3);
						for(int t=0;t<i;t++)
						{
							String tes=(String)QuoTb2.getValueAt(t,0);
							qty1[t]=rs3.getInt(tes);
							//System.out.println("te1"+te1);
							//qty1[t]=qty1[t]+te1;
							QuoTb2.setValueAt(""+qty1[t],t,4);
						}
						retAmt.setText(""+rs3.getDouble(3));
					}
					else
					{
						for(int t=0;t<i;t++)
