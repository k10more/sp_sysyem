...
import javax.swing.border.*;
 ...
 class MisOrdPen extends JFrame  implements ActionListener 
{
	...
	*/
	...
	{	
		...
		*/
		...
		try
		{
			...
			rs=st.executeQuery("select  DISTINCT POrder_Date from POrder order by POrder_Date ");
			while(rs.next())
			{
	...
	public void actionPerformed(ActionEvent e)
	{
			...
			else
			{
				...
				else
				{
					...
					//if(supNm.equals(no)  &&  OrdSta.equals(no))
					{
						try
						{
							rs2=st2.executeQuery("select * from POrder where  POrder_Date  between  '"+dtFrom+"'   and  '"+dtTo+"'  ");
							while(rs2.next())
							{
			...
			//e1.showMessageDialog(null,se4.getMessage(),"Error",JOptionPane.ERROR_MESSAGE);
					...
					//if(!(supNm.equals(no)  &&  OrdSta.equals(no)))
					{
						...
						try
						{
							...
							rs2=st2.executeQuery("select * from POrder where POSupl_Id='"+suptemp+"'  and  POStatus="+statemp+"  and  POrder_Date  between  '"+dtFrom+"'   and  '"+dtTo+"'  ");
							while(rs2.next())
							{
								ONo.addItem(rs2.getString(1));
								recCnt++;
					...
					//if( !(supNm.equals(no))  &&  OrdSta.equals(no) && b==true )
					{
						...
						//{
							try
							{
								...
								rs2=st2.executeQuery("select * from POrder where POSupl_Id='"+suptemp+"'  and    POrder_Date  between  '"+dtFrom+"'   and  '"+dtTo+"'   and not  POStatus=3 ");
								while(rs2.next())
								{
									ONo.addItem(rs2.getString(1));
									recCnt++;
							...
							{
								//JOptionPane e1=new JOptionPane();
			//e1.showMessageDialog(null,se5.getMessage(),"Error",JOptionPane.ERROR_MESSAGE);
							}
					}
					if(num3==0 && num4!=0)
					//if(!(OrdSta.equals(no))  && supNm.equals(no)  && b==true)
						{
							try
							{
								...
								rs2=st2.executeQuery("select * from POrder where POStatus="+statemp+"  and    POrder_Date  between  '"+dtFrom+"'   and  '"+dtTo+"'  ");
							...
							{
								//JOptionPane e1=new JOptionPane();
			///e1.showMessageDialog(null,se6.getMessage(),"Error",JOptionPane.ERROR_MESSAGE);
							}
		...
		if(e.getSource()==rpt)
		{
			...
			ReportPOrd b = new ReportPOrd(ori,pnm,ppri,qord,qinv,qpen,qret);
		}
		...
		if(e.getSource()==ONo)
		{
			...
			try
			{
				...
				if(no!=0 )
				{
					...
					rs4=st4.executeQuery("select  Supl_Name from Supplier  where  Supl_Id='"+rs2.getString(3)+"'");
					rs4.next();
